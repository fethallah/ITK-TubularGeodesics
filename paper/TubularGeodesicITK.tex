...
tracer that uses the fast marching algorithm. The framework requires a minimal amount of user interaction to trace
tubular tree-like structures such as vessels and neurites in 2D images and 3D stacks.

Our method is a minimal path technique that models the vessel as a sequence of centerline points and radius values. 
The crucial step of our method is the definition of the local tubularity metric to minimize. For that, we propose an 
implementation of the oriented flux measure to drive the propagation of the fast marching algorithm. 
...
We deal with the problem of finding a complete segmentation of tubular structures in 2D images and 3D stacks. The main objective is to extract the centerline coordinates of the tubular structures and their thickness. The proposed method is interactive, requiring only a start and an end point to trace a \textit{geodesic tube} between them.

We use  a variant of the  minimal path method   applied  in   the   scale-space   domain~\cite{Li07}. 
As shown in the diagram of figure \ref{fig:diagram}, we first compute a \textit{tubularity} value at each image location $x_i$ and radius value $r_i$, which quantifies the likelihood that there exists a tubular structure of radius $r_i$, at location $x_i$. Given an N-D image, this creates an (N+1)-D scale-space tubularity measure, which, in this work, is taken as the sum of the two dominant eigenvalues of the Optimally Oriented Flux matrix~\cite{Law08}. This step is followed by computation of a distance map from a given start point using the Fast Marching algorithm~\cite{Sethian99}. Finally, the geodesic tube is extracted by back-propagating from an end point to the start. The method presented here is a simplified version of \cite{Benmansour11} since we consider only isotropic tubularity measures.
...
Compared to the ITK implementation proposed in~\cite{Mueller2008}, which provides only centreline locations, our method provides, at the same time, radius estimates. Moreover, our back-propagation step uses the $4^{th}$ order Runge-Kutta optimizer, and hence, yields less oscillations. Finally, in~\cite{Mueller2008} the author claims that \lq\lq choosing an appropriate speed function is the most difficult part of the entire process\rq\rq. We propose an implementation that provides a well suited speed function to extract tubular structures.

This implementation have been incorporated in the \texttt{ImageJ/FIJI} plugin \textit{Simple Neurite Tracer} \footnote{\url{http://cvlab.epfl.ch/software/delin/index.php}} (SNT) for semi-automatic tracing of tubular or ridge-like structures as illustrated in figure~\ref{fig:snt}.
...
%Caption
\vspace{-0.5cm}
\caption{Our extended SNT tracer uses the ITK implementation of the tubular geodesic extractor.}
%Caption
\label{fig:snt}
\end{figure}
%FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF
...
In \cite{Li07}, a variant of the classical, purely spatial, minimal path technique is proposed. By incorporating an extra \textit{non-spatial} dimension into the search space, the method models a tubular path in a 3D image as a sequence of 4D points (three spatial dimensions for coordinates and a scale dimension that quantifies the vessel thickness at that corresponding 3D point). Thus, each 4D point represents a sphere in 3D space, and the vessel is obtained by taking the envelope of these spheres as we move along the 3D curve (see figure~\ref{fig:nDVesselAs} for a 2D example). 

A crucial step of this method is building an informative tubularity measure that drives the propagation. As opposed to \cite{Li07} that requires a few parameters to tune, the oriented flux-based tubularity measure is parameter free.
%FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF
\begin{figure}[!h]
\begin{center}		
...
\end{center}
%Caption
\caption{A tubular shape is represented as an envelope of a family of spheres with continuously changing center points and radii.}
%Caption
...
The tubularity measure we use is a function of the Optimally Oriented Flux (OOF) matrix \cite{Law08}.
...
\end{equation}
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
where $G$ is a Gaussian function with a sufficiently small standard deviation, $r$ is the radius of the sphere (or circle), $\mathbf{h} = r \mathbf{n}$ is the relative position vector along $\partial S_r$, with $\mathbf{n}$ the outward unit normal of $\partial S_r$, and $\mathrm{d}a$ is the infinitesimal area (or length) on $\partial S_r$. Function $f$ is the flux of the smoothed image gradient $\nabla(G*I)$ projected along direction $\mathbf{v}$ towards the sphere $\partial S_r$. To detect vessels having higher intensity values than the background, one would be interested in finding the vessel direction which minimizes $f(\mathbf{x}, \mathbf{v}; r)$, i.e. we are looking for:
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\begin{equation}
...
By differentiating the above equation with respect to $\mathbf{v}$, minimization of function $f$ is in turn acquired as solving a generalized eigenvalue decomposition problem. Solving the aforementioned generalized eigen decomposition problem gives $N$ eigenvalues for an N-D image, $\lambda_1(\cdot)\leq \dots\leq \lambda_{N}(\cdot)$. To handle the tubular structures with varying thickness, we follow the multi-scale approach of Law and Chung~\cite{Law08}, which normalizes the  eigenvalues of the OOF matrix by the surface area of the sphere ($4\pi r^2$). In the 2D case, the eigenvalues are normalized by the circle perimeter $2\pi r$.

The normalized convolution $I*\mathbf{F}_r/r^{N-1}$ step is implemented in  the \code{itk::OrientedFluxMatrixImageFilter} class. This filter is templated over the input image type and the output image type for which the default pixel type is \code{itk::SymmetricSecondRankTensor}, since the OOF provides a symmetric matrix. The filter \code{itk::MultiScaleTubularityMeasureImageFilter} implements a multi-scale oriented flux measure by summing the $(N-1)$ eigenvalues of the OOF matrix that correspond to the cross-section of the tubular structures. In addition to the  (N+1)-D tubularity measure, the filter optionally outputs scale and OOF matrix images (see \code{itkMultiScaleTubularityMeasureImageFilter.cxx} for a complete example).
...
\begin{figure}[t]
  ...
  spiral shape, a 2D cerebral angiogram, a 3D stack of olfactory projection fibers  (OP) from the DIADEM  competition \cite{Diadem10}. Second row: tracing results overlaid on top of the images. Each branch in the 
...
We've evaluated our pipeline on a large variety of noisy 2D images and 3D images stacks. Figure~\ref{fig:results}  
shows three sample results on a synthetically generated image, a 2D cerebral angiogram and a 3D stack of neurite fibers. Maximum intensity projections of the tubularity stacks for each scale level are shown in figure~\ref{fig:mip_tubularity_images}. 

Below we show an example usage of the tubularity and geodesic computation steps for the 3D stack shown in figure~\ref{fig:results}.
...
\begin{figure}[t]
  ...
  tubularity volumes for the OP stack shown in the last column of figure~\ref{fig:results}. 
